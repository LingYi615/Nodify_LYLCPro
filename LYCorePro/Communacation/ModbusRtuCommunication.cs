...
namespace LYCorePro.Communacation
{
    /// <summary>
    /// Modbus RTU 通讯实现（串口）
    /// 使用 HslCommunication 库的 ModbusRtu 进行 Modbus RTU 协议通信
    /// </summary>
    public class ModbusRtuCommunication : CommunicationBase
    {
        private ModbusRtu _client;

        public override string ConnectionString => $"Modbus-RTU://{Config.PortName}:{Config.BaudRate}";

        public ModbusRtuCommunication(CommunicationConfig config) : base(config) { }
        ...
        {
            return await Task.Run(() => { try { _client?.Close(); return true; } catch { return false; } });
        }
        ...
        protected override async Task<bool> SendCoreAsync(byte[] data)
        {
            return await Task.Run(() =>
            {
                try { var result = _client.ReadFromCoreServer(data); return result.IsSuccess; }
                catch { return false; }
            });
        }

        protected override async Task<byte[]> ReceiveCoreAsync() => await Task.FromResult<byte[]>(null);
        ...
        public override async Task<string> ReadStringAsync(string address, ushort length)
        {
            ...
            return await Task.Run(() => { var r = _client.ReadString(address, length); return r.IsSuccess ? r.Content : ""; });
        }

        public override async Task<bool> WriteStringAsync(string address, string value)
        {
            if (!IsConnected) return false;
            return await Task.Run(() => { var r = _client.Write(address, value); return r.IsSuccess; });
        }
        ...
        public override void Dispose() { base.Dispose(); _client?.Dispose(); }
    }
}