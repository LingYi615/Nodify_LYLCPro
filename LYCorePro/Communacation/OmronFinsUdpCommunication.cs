...
namespace LYCorePro.Communacation
{
    ...
    /// 欧姆龙 FINS UDP 通讯实现
    /// </summary>
    public class OmronFinsUdpCommunication : CommunicationBase
    {
        private OmronFinsNet _client;

        public override string ConnectionString => $"FINS-UDP://{Config.RemoteIP}:{Config.RemotePort}";

        public OmronFinsUdpCommunication(CommunicationConfig config) : base(config)
        {
        }
        ...
        protected override async Task<bool> ConnectCoreAsync()
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    // 尝试设置UDP模式（某些版本可能不支持）
                    var prop = _client.GetType().GetProperty("IsUdp");
