...
namespace LYCorePro.Communacation
{
    ...
    /// 支持 TCP 连接断开检测
    /// </summary>
    public class BeckhoffAdsCommunication : CommunicationBase
    {
        private BeckhoffAdsNet _client;

        public override string ConnectionString => $"ADS://{Config.RemoteIP}:{Config.RemotePort}";

        public BeckhoffAdsCommunication(CommunicationConfig config) : base(config)
        {
        }
        ...
        protected override async Task<bool> ConnectCoreAsync()
        {
            return await Task.Run(() =>
            {
                try
                {
                    _client = new BeckhoffAdsNet(Config.RemoteIP, Config.RemotePort);
                    var result = _client.ConnectServer();
                    return result.IsSuccess;
                }
                catch (Exception ex)
                {
                    // 使用基类的 RaiseErrorOccurred 方法
