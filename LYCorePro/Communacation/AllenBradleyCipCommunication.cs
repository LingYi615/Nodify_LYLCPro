...
{
    /// <summary>
    /// 罗克韦尔 CIP (EtherNet/IP) 通讯实现
    /// 支持 TCP 连接断开检测
    /// </summary>
    public class AllenBradleyCipCommunication : CommunicationBase
    {
        private AllenBradleyNet _client;

        public override string ConnectionString => $"CIP://{Config.RemoteIP}:{Config.RemotePort}";

        public AllenBradleyCipCommunication(CommunicationConfig config) : base(config)
        {
        }
        ...
        protected override async Task<bool> ConnectCoreAsync()
        {
            return await Task.Run(() =>
            {
                try
                {
                    _client = new AllenBradleyNet(Config.RemoteIP, Config.RemotePort);
