...
namespace LYCorePro.Communacation
{
    ...
    /// 支持 TCP 连接断开检测
    /// </summary>
    public class SiemensS7Communication : CommunicationBase
    {
        private SiemensS7Net _client;
        private SiemensPLCS _plcType = SiemensPLCS.S1200;

        public override string ConnectionString => $"S7://{Config.RemoteIP}:{Config.RemotePort}";

        public SiemensS7Communication(CommunicationConfig config) : base(config)
        {
        }
        ...
        protected override async Task<bool> ConnectCoreAsync()
        {
            return await Task.Run(() =>
            {
                try
                {
                    _client = new SiemensS7Net(_plcType, Config.RemoteIP);
