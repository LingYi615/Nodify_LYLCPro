...
namespace FunctionBlockPlugin
{
    ...
    /// - 不包含任何逻辑运算，仅作为容器/子图
    /// </summary>
    public class FunctionBlockOperationViewModel : NotifyPropertyBase, IExecutableOperation, ISerializableOperation, IConnectionAware
    {
        #region 静态资源

        /// <summary>可用数据类型（排除 Any）</summary>
        ...
        /// <summary>输入连接器（初始为空，需通过参数面板新增）</summary>
        ...
        public void OpenSubEditor()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("[FunctionBlock.OpenSubEditor] ParentEditor 未注入，无法展开");
                return;
            }
            ...
            System.Diagnostics.Debug.WriteLine($"[FunctionBlock.OpenSubEditor] 已展开子画布: {Title}");
        }

        /// <summary>
        /// 同步入口/出口节点到子画布（增量更新，保留已有连线）
        /// 入口节点对应 FunctionBlock 的 Inputs，出口节点对应 FunctionBlock 的 Outputs
        /// </summary>
        ...
        private bool ExecuteInternal()
        {
            try
            {
                ...
                // 同步入口/出口节点
                SyncEntryExitNodes();
                // 恢复反序列化时暂存的连线数据（未展开子画布直接运行时需要）
                RestoreSubEditorConnections();

                // 将 FunctionBlock 的输入值传递到子画布入口节点
                var entryNodes = _subEditorViewModel.Operations
                    .Where(n => n.Instance is SubEditorEntryNode)
                    .ToList();
                // 从子画布出口节点读取值到 FunctionBlock 输出
                var exitNodes = _subEditorViewModel.Operations
                    .Where(n => n.Instance is SubEditorExitNode)
                    .ToList();
                ...
                // 执行子画布中所有非入口/出口节点（按拓扑排序确保数据正确传递）
                ...
                //StatusMessage = $"执行: {executed}/{allNodes.Count}";
                RunStatusMessage(NodeRunStatus.Completed, "执行成功");
                return executed > 0 || allNodes.Count == 0;
            }
            catch (Exception ex)
            {
        ...
        private void RestoreSubEditorConnections()
        {
            ...
            System.Diagnostics.Debug.WriteLine(
                $"[FunctionBlock.RestoreSubEditorConnections] 已恢复子画布连线");
        }
    ...
    /// <summary>
    /// 子画布入口节点（数据从 FunctionBlock Input 流入，向内部节点输出）
    /// </summary>
    ...
    /// <summary>
    /// 子画布出口节点（从内部节点接收数据，流向 FunctionBlock Output）
    /// </summary>
